...
for ii=1:cnt_cross
    ...
    switch metric
        case 'Robust_CIQA'     
            ...
            isShow=0;
            isBestParam=1;
        case 'Robust_CIQA_NoNorm'  % 没有标准化处理
            start=2; % 特征起点
            end2=1; % 特征终点
            train_data=my_mat(inds_train,:);
            test_data=my_mat(inds_test,:); 
...
%             isShow=0;
            ...
            model=svmtrain(train_data_ideal(:,end),train_data_ideal(:,start:end-end2),sprintf('-s %f -t %f -c %f -g %f', 3, 2, best_C, best_gamma)); % add -v 交叉验证，不输出模型值
            %% svr 预测
